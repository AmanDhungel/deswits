...
export async function issueOtp(email: string, fullName?: string): Promise<IssueOtpResult> {
...
export async function verifyOtp(email: string, code: string): Promise<VerifyOtpResult> {
  ...
  if (!record) {
    return { ok: false, error: "Code not found or already used. Request a new one." };
  }
